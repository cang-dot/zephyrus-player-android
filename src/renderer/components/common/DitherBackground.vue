...
/**
 * DitherBackground — 黑白可见性抖动背景（OGL + fbm 域扭曲 + Bayer 8×8 有序抖动）
 * 来源：vue-bits (https://vue-bits.dev/backgrounds/dither, MIT License)，移植着色器与骨架。
 * 本地适配：paused 外部暂停 + IntersectionObserver（上游只有 window.resize）、dpr/帧率上限、
 * 移除上游的 deep watch（任何 prop 变化都会重建场景；每帧 update 已在同步 uniform，无需重建）、
 * 初始化失败 emit('fallback')。压暗由父级在画布上叠暗层实现，保持色阶纯净。
 */
...
const shouldRun = () => isVisible && !isExternallyPaused && !document.hidden;
...
const initializeScene = () => {
  ...
  try {
    ...
    console.warn('[DitherBackground] WebGL 初始化失败，回退 CSS 背景', error);
    emit('fallback');
  }
};
...
onMounted(() => {
  isExternallyPaused = Boolean(props.paused);
  initializeScene();
});
...
watch(
  () => props.paused,
  (value) => {
    isExternallyPaused = Boolean(value);
    if (isExternallyPaused) stop();
    else start();
  }
);
</script>
