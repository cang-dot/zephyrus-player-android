...
/**
 * LiquidEther — WebGL 流体模拟背景
 * 来源：vue-bits (https://vue-bits.dev/backgrounds/liquid-ether, MIT License)
...
interface LiquidEtherProps {
  ...
  /** 渲染 dpr 上限（默认 2）；流体为模糊美学，移动端建议 1.5 */
...
const initWebGLUnsafe = () => {
  if (!mountRef.value) return null;
  ...
  let paletteTex = makePaletteTexture(props.colors);
  ...
  class CommonClass {
    ...
    init(container: HTMLElement, maxPixelRatio = 2) {
  ...
  class MouseClass {
    ...
    init(container: HTMLElement) {
      this.container = container;
      // 播放器场景不监听任何指针事件：触摸交互会与播放器的
      // 单击切控件/滑动关面板/歌词滑动手势冲突，流体运动完全交给 AutoDriver
    }
  ...
  class Simulation {
    ...
    getFloatType(): THREE.TextureDataType {
      // 安卓 WebView 普遍缺少 OES_texture_float_linear，强制 FloatType 会导致流体黑屏，
  ...
  class WebGLManager {
    ...
    output!: Output;
    paletteTex: THREE.DataTexture = paletteTex;
    ...
    constructor(config: WebGLManagerConfig) {
      this.config = config;
      Common.init(config.$wrapper, config.maxPixelRatio);
    ...
    init() {
      ...
      Common.renderer.domElement.addEventListener('webglcontextlost', this._onContextLost, false);
    }
    ...
    /** 源组件不支持 colors 热更新；播放器需要随歌曲主题色实时换调色板 */
    applyPalette(stops: string[]) {
      const next = makePaletteTexture(stops);
      if (this.output) {
        (this.output.outputMesh.material as THREE.RawShaderMaterial).uniforms.palette.value = next;
      }
      this.paletteTex.dispose();
      this.paletteTex = next;
    }
    ...
    loop() {
      ...
      const now = performance.now();
      const minInterval = this.config.maxFps > 0 ? 1000 / this.config.maxFps : 0;
      // 帧率上限 + 播放器开合变形期让帧（与 Smoke/Rain/Glitch 背景同款惯例）
      if (
        (minInterval > 0 && now - this._lastFrameAt < minInterval) ||
        shouldSkipMobilePlayerFrame(this._lastFrameAt, now)
      ) {
...
let activeWebGL: ReturnType<typeof initWebGLUnsafe> | null = null;

/** WebGL 初始化失败时静默降级为静态深色背景，不让播放器崩溃 */
const initWebGL = () => {
  try {
    return initWebGLUnsafe();
  } catch (err) {
    // Common/renderer 均在 Unsafe 闭包内，失败场景交由浏览器回收 context
    console.warn('[LiquidEther] WebGL init failed, fallback to static background:', err);
    return null;
  }
};
...
// 源组件不支持 colors 热更新；播放器场景需要随歌曲主题色实时换调色板
watch(
  () => props.colors.join(','),
  () => {
    activeWebGL?.applyPalette(props.colors);
  }
);

// 外部暂停（减弱动画）：画面静止在当前帧，恢复时续跑
watch(
  () => props.paused,
  (paused) => {
    if (!activeWebGL) return;
    if (paused) activeWebGL.pause();
    else if (!document.hidden && isVisibleRef.value) activeWebGL.start();
  }
);
