          ...
          >
            <div class="card-inner">
              ...
              <div
                ...
                class="card-remove"
                @pointerdown.stop
                @click.stop="removeItem(item.type, 'card')"
              >
                ...
                <div class="card-content card-daily">
                  <div class="card-top-row">
                    <span class="card-badge"
                      ><i class="ri-calendar-check-fill" />{{ dailyCount
                      }}{{ t('comp.homeHero.songs') }}</span
                    >
                ...
                <div class="card-content card-user">
                  <div class="card-body user-body">
                    ...
                    <div class="user-info">
                      ...
                      <p v-if="isLoggedIn" class="card-subtitle">
                        {{ t('comp.modularHome.viewProfile') }}
                      </p>
              ...
              <template v-else-if="item.type === 'artists'">
                <div class="card-content card-artists">
                  <div class="card-top-row">
                    <span class="card-badge"
                      ><i class="ri-mic-fill" />{{ t('comp.homeHero.hotArtists') }}</span
                    >
                  </div>
                  <div class="card-body">
              ...
              <template v-else-if="item.type === 'playlists'">
                <div class="card-content card-playlists">
                  <div class="card-top-row">
                    <span class="card-badge"
                      ><i class="ri-play-list-2-fill" />{{ t('comp.homeHero.hotPlaylists') }}</span
                    >
                  </div>
                  <div class="card-body">
              ...
              <template v-else-if="item.type === 'daily-album'">
                <div class="card-content card-daily-album">
                  <div class="card-top-row">
                    <span class="card-badge"
                      ><i class="ri-disc-fill" />{{ t('comp.modularHome.blocks.dailyAlbum') }}</span
                    >
                  </div>
                  <div class="card-body daily-album-body">
                    ...
                    <div class="daily-album-info">
                      ...
                      <p class="card-subtitle">
                        {{ dailyAlbum?.artist?.name || t('comp.modularHome.dailyAlbumDesc') }}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <template v-else>
                <div class="card-content card-static">
                  <div class="card-top-row">
                    <span class="card-badge"
                      ><i :class="meta[item.type].icon" />{{ t(meta[item.type].titleKey) }}</span
                    >
                  </div>
                  <div class="card-body">
                    <h2 class="card-title">{{ t(meta[item.type].titleKey) }}</h2>
                    <p class="card-subtitle">
                      {{ t(meta[item.type].descKey || 'comp.modularHome.tapToExplore') }}
                    </p>
                  </div>
      ...
      <div ref="blockGridRef" class="block-grid-container">
        ...
        >
          ...
          <div
            ...
            class="block-remove"
            @pointerdown.stop
            @click.stop="removeItem(item.type, 'grid')"
          >
            <i class="ri-close-circle-fill" />
          </div>
          <div
            v-if="isEditMode"
            class="resize-handle"
            @pointerdown.stop.prevent="onResizeStart($event, item)"
          >
        ...
        >
          ...
          <div class="block-inner add-inner">
            <i class="ri-add-line" /><span>{{ t('comp.modularHome.addBlock') }}</span>
          </div>
        </div>
      </div>
...
> = {
  ...
  user: {
    ...
    gradient:
      'linear-gradient(135deg, var(--accent-color-light, #6366f1), var(--accent-color, #4f46e5))'
  },
  artists: {
    icon: 'ri-mic-fill',
    ...
    gradient:
      'linear-gradient(135deg, var(--accent-color-dark, #f59e0b), var(--accent-color, #d97706))'
  },
  playlists: {
    ...
    gradient:
      'linear-gradient(135deg, var(--accent-color, #8b5cf6), var(--accent-color-dark, #7c3aed))'
  },
  'daily-album': {
    icon: 'ri-disc-fill',
    ...
    gradient:
      'linear-gradient(135deg, var(--accent-color-light, #14b8a6), var(--accent-color, #0d9488))',
    requiresLogin: true
  },
  toplist: {
    icon: 'ri-trophy-fill',
    ...
    gradient:
      'linear-gradient(135deg, var(--accent-color-dark, #eab308), var(--accent-color, #ca8a04))'
  },
  albums: {
    icon: 'ri-album-fill',
    ...
    gradient:
      'linear-gradient(135deg, var(--accent-color-light, #06b6d4), var(--accent-color, #0891b2))'
  },
  history: {
    ...
    gradient:
      'linear-gradient(135deg, var(--accent-color, #10b981), var(--accent-color-dark, #059669))'
  },
  favorites: {
    ...
    gradient:
      'linear-gradient(135deg, var(--accent-color-light, #ec4899), var(--accent-color, #db2777))'
  },
  podcast: {
    ...
    gradient:
      'linear-gradient(135deg, var(--accent-color-dark, #6366f1), var(--accent-color, #4f46e5))'
  },
  mv: {
    ...
    gradient:
      'linear-gradient(135deg, var(--accent-color, #ef4444), var(--accent-color-dark, #dc2626))'
  }
};
...
const loadLayout = (): { cards: LayoutItem[]; blocks: LayoutItem[] } => {
        ...
        const toItems = (arr: any[]) =>
          arr
            ...
            .map((b: any) => ({
              ...
              w: Math.min(4, Math.max(1, b.w || 2)),
              h: Math.min(2, Math.max(1, b.h || 2))
            }));
    ...
    if (old) {
      ...
      if (parsed && Array.isArray(parsed.cards) && Array.isArray(parsed.blocks)) {
        const toItems = (arr: string[]) =>
          arr
            .filter((b: string) => ALL_BLOCKS.includes(b as BlockType))
            .map((type: string) => ({ type: type as BlockType, w: 2, h: 2 }) as LayoutItem);
        return { cards: toItems(parsed.cards), blocks: toItems(parsed.blocks) };
      }
    }
  } catch {
  ...
  return {
    cards: [
      { type: 'daily-recommend', w: 2, h: 2 },
      { type: 'personal-fm', w: 2, h: 2 }
    ],
    blocks: [
      { type: 'user', w: 2, h: 1 },
      { type: 'artists', w: 2, h: 2 },
      { type: 'playlists', w: 2, h: 2 },
      { type: 'daily-album', w: 2, h: 1 },
      { type: 'toplist', w: 2, h: 2 }
    ]
  };
};
...
const saveLayout = () => {
  try {
    localStorage.setItem(
      'homeLayoutV2',
      JSON.stringify({ cards: cardItems.value, blocks: blockItems.value })
    );
  } catch {
    /* ignore */
  }
...
const availableBlocks = computed(() =>
  ALL_BLOCKS.filter(
    (b) => !cardItems.value.some((i) => i.type === b) && !blockItems.value.some((i) => i.type === b)
  )
);
...
const displaySong = computed(() =>
  activeMode.value === 'intelligence' ? playerCoreStore.currentSong : fmCurrentSong.value
);
...
const openDailyAlbum = () => {
  if (dailyAlbum.value) router.push(`/music-list/${dailyAlbum.value.id}?type=album`);
};
...
const onBlankPointerDown = (e: PointerEvent) => {
  ...
  if (
    ...
    target.closest('.block-item') ||
    target.closest('.add-sheet-item')
  ) {
    return;
  }
...
const onDocPointerMove = (e: PointerEvent) => {
  ...
  const selector =
    drag.source === 'card' ? `[data-card-id="${drag.itemId}"]` : `[data-block-id="${drag.itemId}"]`;
  ...
  if (drag.source === 'grid' && isCardZone) {
    ...
    if (drag.targetZone !== 'grid') {
      ...
      if (el) {
        el.style.transition =
          'width 0.35s cubic-bezier(0.34, 1.56, 0.64, 1), height 0.35s cubic-bezier(0.34, 1.56, 0.64, 1), border-radius 0.3s ease';
  ...
  if (drag.source === 'card' && drag.targetZone === 'card') {
    ...
    const col = Math.floor((e.clientX - gridRect.left) / colWidth);
    const row = Math.floor((e.clientY - gridRect.top) / rowHeight);
    const cellIndex = Math.max(0, Math.min(blockItems.value.length - 1, row * 4 + col));
    const fromIndex = blockItems.value.findIndex((item) => item.type === drag.itemId);
    if (fromIndex === -1 || cellIndex === fromIndex) return;

    const beforeRects = recordRects(grid, '.block-item:not(.dragging):not(.add-block)');
    const moved = blockItems.value.splice(fromIndex, 1)[0];
    ...
    if (draggedElement) {
      const oldRect = draggedElement.getBoundingClientRect();
      nextTick(() => {
        const newElement = document.querySelector(dragSelector) as HTMLElement | null;
        if (newElement) {
...
const onResizeStart = (e: PointerEvent, item: LayoutItem) => {
  ...
  resizeEndHandler = () => {
    ...
    if (resizeEndHandler) {
      document.removeEventListener('pointerup', resizeEndHandler);
      document.removeEventListener('pointercancel', resizeEndHandler);
...
.orb-2 {
  width: 280px;
  height: 280px;
  background: var(--accent-color-dark, #8b5cf6);
  top: 35%;
  right: -15%;
  animation-delay: -8s;
}
.orb-3 {
  width: 240px;
  height: 240px;
  background: var(--accent-color-light, #06b6d4);
  bottom: 15%;
  left: 25%;
  animation-delay: -16s;
}
...
.add-sheet-thumb-icon {
  font-size: 32px;
  opacity: 0.9;
}
.add-sheet-thumb-label {
  font-size: 13px;
  font-weight: 600;
  text-shadow: 0 1px 8px rgba(0, 0, 0, 0.2);
}
...
.sheet-leave-active {
  ...
  .add-sheet {
    transition: transform 0.3s var(--m-ease-drawer, cubic-bezier(0.32, 0.72, 0, 1));
  }
}
.sheet-enter-from,
.sheet-leave-to {
