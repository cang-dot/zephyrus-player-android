  ...
  <section class="daily-recommend-section">
    ...
    <div class="section-header mb-6 md:mb-8 flex items-end justify-between">
      <div>
        ...
        <div class="mt-1.5 h-1 w-12 rounded-full bg-[var(--accent-color)]" />
      </div>
      <div class="flex items-center gap-3">
        <!-- Play All Button -->
        <button
          v-if="!loading && songs.length > 0"
          class="play-all-btn flex items-center gap-1.5 text-xs md:text-sm font-bold text-[var(--accent-color)] dark:text-white hover:text-[var(--accent-color)]/80 dark:hover:text-white/80 transition-colors"
          @click="playAll"
        >
    ...
    >
      ...
      >
        ...
        <div class="song-info mt-3 md:mt-4 px-0.5">
          <h3
            class="song-name line-clamp-2 text-sm md:text-base font-semibold leading-tight text-neutral-800 dark:text-neutral-100 transition-colors duration-200 group-hover:text-[var(--accent-color)] dark:group-hover:text-white"
          >
            {{ song.name }}
          </h3>
          <p
