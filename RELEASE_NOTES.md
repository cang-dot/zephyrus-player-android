...
- **Spotify 登录集成**：支持 Spotify OAuth 2.0 PKCE 登录，可在登录页直接选择 Spotify 登录
- **多平台账号切换**：「我的」页长按头像触发账号切换，卡片向下拉长展示其他平台账号，统计栏与标签栏渐隐
- **设置页重构**：移除独立 Spotify 设置选项卡；解锁密钥移至基础设置，意见反馈移至关于页面
- **骨架屏优化**：「我的」页加载占位改为旋转图标 + 遮罩文字，取代灰色色块
- **Spotify 搜索与歌单**：登录后可直接搜索 Spotify 曲库并浏览歌单

### 🎨 优化

- **设置页默认滚动**：打开设置页时自动向下滚动，避开卡片直接展示分区内容
- **我的页默认滚动**：加载后与占位页位置一致，避免顶栏遮挡
- **账号切换动画**：弹性缓动改为 Material 标准 ease，动画更平稳

### 🐛 修复

- **账号切换无效**：修复点击其他账号后仍显示原账号的问题（缺少 `setActiveAccount` 调用）
- **账号选择器点击穿透**：修复 backdrop 层级过高导致点击账号列表内按钮被拦截的问题
...
- **后台下载**：点击「立即更新」后按钮自动变为「后台下载」，可随时切换为后台继续下载，下载完成后再次打开更新弹窗直接显示「立即安装」
- **原生线程下载更新包**：APK 改为 Java 原生线程直写缓存，避免大文件经 JS 桥接导致的内存溢出闪退
- **服务器 RELEASE_NOTE 自动同步**：服务器网关启动及定时拉取 GitHub Release 更新日志，自动生成 `latest.json`

### 🐛 修复

- **唤起系统安装器失败**：显式定位 Google / Android / ColorOS / Oplus / OnePlus / 华为 / MIUI 各厂商包安装器并授权 URI，兜底系统选择器，解决「点击用软件包安装程序打开却没有跳转」
- **更新检查服务器优先**：优先读取服务器 `latest.json`，兼容 `-beta` 预发布版本号比较
